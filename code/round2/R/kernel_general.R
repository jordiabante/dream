...
cell_line_order=read.table(cell_line_order_path,sep=",",header=F)
drug_combination_order=read.table(drug_combination_order_path,sep=",",header=F)
...
# Write output
write.table(kernel_total_train,file=paste("../../data/round2/kernel_train_test/train_",outsuffix,".txt"),col.names=F,row.names=F,sep="\t",quote=F)
write.table(kernel_test,file=paste("../../data/round2/kernel_train_test/test_",outsuffix,".txt"),col.names=F,row.names=F,sep="\t",quote=F)

print(paste("train and test kernel for "),outsuffix," completed"))
