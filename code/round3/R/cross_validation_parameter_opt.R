...
################################################################
ch1_train <- read.csv("../../dream/data/originals/ch1_train_combination_and_monoTherapy.csv", stringsAsFactors=F,sep=",",header=T)
...
for(l1 in 1: length(eps_range)){
  for(l2 in 1: length(C_range)){
    for(l3 in 1: length(nu_range)){
      ...
      output <- cbind(kernel_train_path,1,eps_range[l1],C_range[l2],nu_range[l3],err_mse,err_global,err_mean)
...
for(l in 1: length(lambda_range)){
  ...
  output <- cbind(kernel_train_path,2,lambda_range[l],err_mse,err_global,err_mean,err_mse_weight,err_global_weight,err_mean_weight)
  write.table(output,stdout(),sep="\t",row.names=F,col.names=F,quote=F)
}
#}
#}