...
make_kernel <- function(kernel_cell_line,kernel_drug,outsuffix){  
  ...
  # Write output
  write.table(kernel_total_train,file=paste("../../data/round2/kernel_train_test_product/kernel_train_",outsuffix,".txt",sep=""),col.names=F,row.names=F,sep="\t",quote=F)
  write.table(kernel_test,file=paste("../../data/round2/kernel_train_test_product/kernel_test_",outsuffix,".txt",sep=""),col.names=F,row.names=F,sep="\t",quote=F)
