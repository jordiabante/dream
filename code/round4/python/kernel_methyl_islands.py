...
def Kernel():
	...
	for m in range(0,cell_len):
		for n in range(0,cell_len):
			...
			kernel_cor[m,n] = t[0]
	np.savetxt('../../data/round4/kernels/dot_product_methylation_islands.txt',kernel_dot, delimiter = '\t')
	np.savetxt('../../data/round4/kernels/corr_methylation_islands.txt',kernel_cor, delimiter = '\t')	
	np.savetxt('../../data/round4/kernels/angular_similarity_methylation_islands.txt',kernel_ang, delimiter = '\t')			
